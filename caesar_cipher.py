...
def get_coder(line_old, offset):
    """
    Перекодируем строку текста по ключу. На вход подается строка текста, ключ кодирования.
    Язык строки определяется автоматически по первому символу.
...
def get_parse_string(text_old):
    """
    Разбираем строку текста. На вход подается строка текста. На выходе получаем список.
...
def get_concatenate_string(string_list):
    """
    Получаем строку текста из списка. На вход подается список. На выходе получаем строку текста.
...
while answer:
    ...
    mode = 1 if get_polls('Выберите режим: шифровать / дешифровать текст', True, 'ш', 'д') else -1
    # Направление кодировки.
    route = 1 if get_polls('Выберите направление кодировки (вправо / влево)', True, 'п', 'л') else -1
    # Текст для обработки.
    ...
    answer = get_polls('Обработать еще один текст?', False, 'н', 'д')
